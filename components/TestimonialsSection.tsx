    ...
    <section id="testimonials" className="py-24 px-8 max-w-screen-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {featured.map((t) => (
          <div
            key={`${t.name}-featured`}
            className="liquid-glass p-8 relative border-b-2 border-primary/30"
          >
            <span className="absolute top-4 right-5 font-mono text-5xl text-outline-variant opacity-10 select-none leading-none">
              &quot;
            </span>
            <StarRating />
            <p className="text-on-surface mb-8 italic leading-relaxed">
              &quot;{t.quote}&quot;
            </p>
            <div className="flex items-center gap-4 border-t border-outline-variant/20 pt-5">
              <div
                className={`w-9 h-9 rounded-sm liquid-glass flex items-center justify-center font-mono ${t.accentColor} font-bold text-xs`}
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {rest.map((t, i) => (
          <div
            key={`${t.name}-${i}`}
            className="liquid-glass p-5 relative group hover:-translate-y-0.5 transition-transform duration-300"
          >
            <StarRating />
            <p className="text-on-surface-variant text-sm italic mb-5 leading-relaxed">
              &quot;{t.quote}&quot;
            </p>
